...
def sampling_by_tad(background_gene_list, TAD_data, num_selected=3000):
    ...
    TAD_data: [[GeneName, TadGroup], ...], TadGroup is a distinct regulatory state defined by the TAD and histone modification class
    ...
    TAD_data = [(gene_name, tad_group) for (gene_name, tad_group) in TAD_data if gene_name in background_gene_list]

    #collect list of genes in each bin
    genes_in_TAD = defaultdict(set)
    ...
    selected_genes = [
        np.random.choice(genes_in_TAD[tad_group], min(num_genes_in_TAD[tad_group] - 1, num_expected))
        for tad_group, num_expected in expected_samples.items()
    ]

    #flatten the list to yield background gene selections
    return set([gene_name for gene_list in selected_genes for gene in gene_list])
...
def process_user_genelist(user_genelist, gene_symbols, gene_refseqIDs):
    """
    ...
    """
    gene_symbols, gene_refseqIDs = np.array(gene_symbols), np.array(gene_refseqIDs)
    ...
    return list(gene_symbols[user_supplied_genes].values)
...
def create_label_vector(query_list, background_list):
    return query_list + background_list, np.concatenate([np.ones(len(query_list)), np.zeros(len(background_list))])


#converts user-entered data to two gene lists: a query list, and a background list
def process_gene_lists(query_genes, gene_symbols, gene_refseqIDs, TAD_data, num_selected = 3000, user_background_genes = None, method = 'TAD'):
    '''
    ...
    query_list = process_user_genelist(query_genes, gene_symbols, gene_refseqIDs)
    ...
    if not user_background_genes is None:
        background_candidates = process_user_genelist(user_background_genes, gene_symbols, gene_refseqIDs)
    else:
        background_candidates = set(gene_symbols).difference(query_list)
