...
class TestGeneSelection(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.lisa = LISA.LISA('hg38', verbose = False, background_strategy='provided')
        cls.geneset = cls.lisa._load_gene_info()
...
class TestPeakRP_Assay(TestModels):
    ...
    def setUp(self):
        ...
        self.model = LISA_assays.PeakRP_Assay(None, None, 1, utils.Log(verbose = False))
...
class TestAccessibility_Assay(TestGeneSelection):

    def test_accessibility_assay(self):

        results, metadata = self.lisa.predict(self.query_list, self.background_list)

        results = results.subset(0).todict()

        self.assertEqual(results['factor_name'], 'GATA6')
        self.assertTrue(results['DNase_p_value'] < 0.05)
        self.assertTrue(results['H3K27ac_p_value'] < 0.05)
        self.assertTrue(results['ChIP-seq_p_value'] < 0.05)
